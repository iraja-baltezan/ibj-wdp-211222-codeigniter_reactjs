 ...
 *
 * BaseClientForm.js
 * As with the Login and Register components we declare the initial values of
 * the form and our validation schema. The initial values in this component
 * aren’t as straightforward however. This is because when we are editing a
 * client, we want the initial values to correspond with the client’s saved
 * details.
 *
 * There’s also a successCallback which is triggered if the API handled the
 * request successfully. If triggered, the details from the API are saved and a
 * success message is displayed. Additionally, after 4 seconds, the user is
 * returned to the dashboard. This is done using the onTimeout callback prop
 * passed to the SuccessAlert component.
 */
